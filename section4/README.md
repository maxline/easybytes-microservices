...
## 50-51. Loans - Buildpacks approach
...
## 52. Cards - Google Jib approach
https://github.com/GoogleContainerTools/jib
- $ `cards > mvn compile jib:dockerBuild`
- $ `cards > docker images`
- $ `cards > docker run -d -p 9000:9000 sergml/cards:s4`

## 53-60. Docker compose

- $ `accounts > docker pull sergml/cards:s4`

- $ `accounts > docker compose up -d`<br>
Creates and starts containers based on the given Docker Compose file
- $ `accounts > docker compose down`<br>
remove all containers
- $ `accounts % docker compose stop`<br>
stop containers
- $ `accounts % docker compose start`<br>
Starts existing (previously created) containers without recreating them
